...
| `watchdog.py` | The "immortal service" watchdog — heartbeat + self-restart. Part of the launchd + `caffeinate` pattern that ran for 8 days straight and self-healed through sleep/wake. `BOT_DIR`, interpreter, `pg_isready`, and `brew` resolve from env / this snapshot directory (no machine-specific defaults) |
